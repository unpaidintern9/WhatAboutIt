import {
  commitTimelineDraftChange,
  type TimelineCameraDecision,
  type TimelineDraft,
...
export const autoEditModes: AutoEditModeDefinition[] = [
  ...
  },
  {
    ...
    icon: "Zap",
    description:
      "Polish voices, smooth camera choices, and keep conversation natural.",
  },
  {
    id: "fast-paced",
    title: "Fast Paced",
    icon: "Rocket",
    description:
      "Stronger voice polish and direct camera cuts for a tighter rhythm.",
  },
  {
    id: "clip-hunter",
    title: "Clip Hunter",
    icon: "Target",
    description:
      "Polish the episode and prioritize strong highlight suggestions.",
  },
];
...
export function runOfflineAutoEdit(input: {
  ...
  silenceSegments?: AutoEditSilenceSegment[];
  learningProfile?: AutoEditLearningProfile;
}): AutoEditResult {
  const mode = input.mode ?? "balanced";
  const now = input.now ?? new Date().toISOString();
  const originalLengthMs = Math.max(input.draft.durationMs, 0);
  const silenceSuggestions = createSilenceSuggestions(
    input.silenceSegments ?? [],
    mode,
    originalLengthMs,
  );
  const silenceRemovedMs = silenceSuggestions.reduce(
  ...
  const minimumCameraHoldMs = getMinimumCameraHoldMs(
    mode,
    input.learningProfile,
  );
  const cameraDecisions = createCameraDecisions(
    input.activitySegments ?? [],
    now,
    minimumCameraHoldMs,
  );
  const mergedCameraDecisions = mergeAutoCameraDecisions(
    input.draft.cameraDecisions,
    cameraDecisions,
  );
  const silenceOperations: TimelineEditOperation[] = silenceSuggestions.map(
    (segment) => ({
      id: segment.id,
      type: "delete-section",
  ...
  const polishedTracks = input.draft.tracks.map((track) =>
    applyAutoEditProductionPolish(track, mode, input.learningProfile),
  );
    ...
    changesMade: [
      silenceSuggestions.length > 0
        ...
        : {
            id: "timing-safe",
            label:
              "Kept episode timing intact because no long pauses were detected",
            reversible: true,
          },
      {
        id: "markers",
      ...
      { id: "chapters", label: "Suggested chapter markers", reversible: true },
      {
        id: "voice-polish",
        label:
          "Applied podcast voice cleanup, tone, dynamics, and output protection to every saved microphone",
        reversible: true,
      },
      {
        id: "camera-polish",
        label:
          "Applied conservative denoise, color balance, and sharpening to every saved camera",
        reversible: true,
      },
      ...(input.learningProfile
        ? [
        ...
        : []),
      {
        ...
        label:
          mode === "gentle"
            ? "Added short fades through black between camera choices"
            : "Kept direct camera cuts for a clean conversational rhythm",
        reversible: true,
      },
      cameraDecisions.length > 0
        ...
        : {
            id: "camera-program",
            label:
              "Kept the Program camera because separate microphone activity was unavailable",
            reversible: true,
          },
    ],
    audioWarnings: [
      "Listen through each polished microphone and the final mix before export",
    ],
    reviewFlags: [
      "Review automatic camera choices, voice cleanup, picture finishing, and suggested clips before sharing",
    ],
  ...
  const proposedDraft: TimelineDraft = {
    ...
    cameraTransition:
      input.learningProfile?.cameraTransition ??
      (mode === "gentle" ? "fade" : "cut"),
    cameraTransitionMs:
      input.learningProfile?.cameraTransitionMs ??
      (mode === "gentle" ? 300 : 180),
    editLog: [
      ...input.draft.editLog.filter(
        (operation) =>
          operation.type !== "auto-edit-suggestion" &&
          !operation.id.startsWith("auto-silence-"),
      ),
      ...silenceOperations,
      autoOperation,
    ],
  ...
  const draft = commitTimelineDraftChange(
    input.draft,
    proposedDraft,
    `Auto Edit draft (${mode})`,
    now,
  );
...
export function mergeAutoCameraDecisions(
  existing: TimelineCameraDecision[],
  generated: TimelineCameraDecision[],
) {
  const manualDecisions = existing
    .filter((decision) => decision.source === "manual")
    .sort((left, right) => left.startMs - right.startMs);
  if (manualDecisions.length === 0)
    return generated.length > 0 ? generated : existing;

  const firstManualStartMs = manualDecisions[0].startMs;
  const unclaimedAutomaticDecisions = generated.filter(
    (decision) => decision.startMs < firstManualStartMs,
  );
  return [...unclaimedAutomaticDecisions, ...manualDecisions].sort(
    (left, right) => left.startMs - right.startMs,
  );
}

export function applyAutoEditProductionPolish(
  track: TimelineTrack,
  mode: AutoEditMode,
  learningProfile?: AutoEditLearningProfile,
): TimelineTrack {
  ...
  if (track.kind === "camera") {
    ...
    return {
      ...
      ...blendTreatment(
        profile,
        learningProfile?.cameraTreatment,
        learningWeight(learningProfile),
      ),
    };
  }
  return track;
...
function createCameraDecisions(
  activity: AutoEditActivitySegment[],
  now: string,
  minimumCameraHoldMs: number,
) {
  ...
  for (const segment of [...activity].sort((a, b) => a.startMs - b.startMs)) {
    ...
    if (previous && segment.startMs - previous.startMs < minimumCameraHoldMs)
      continue;
    decisions.push({
...
function createSilenceSuggestions(
  segments: AutoEditSilenceSegment[],
  mode: AutoEditMode,
  durationMs: number,
): AutoEditSilenceSuggestion[] {
  ...
  const breathingRoomMs =
    mode === "fast-paced" ? 180 : mode === "gentle" ? 450 : 300;
  return segments
...
export function learnAutoEditProfile(
  ...
  const microphones = draft.tracks.filter(
    (track) => track.kind === "microphone",
  );
  ...
  const manualCuts = draft.cameraDecisions
    .filter((decision) => decision.source === "manual")
  ...
  const gaps = manualCuts
    ...
    .filter((gap) => gap > 0);
  const observedHold =
    gaps.length > 0 ? average(gaps) : (previous?.minimumCameraHoldMs ?? 3500);
  return {
    sampleCount,
    updatedAt: now,
    preferredMode,
    audioTreatment: previous
      ? mergeTreatment(
          previous.audioTreatment,
          audioTreatment,
          previous.sampleCount,
        )
      : audioTreatment,
    cameraTreatment: previous
      ? mergeTreatment(
          previous.cameraTreatment,
          cameraTreatment,
          previous.sampleCount,
        )
      : cameraTreatment,
    ...
    minimumCameraHoldMs: Math.round(
      Math.max(1500, Math.min(12000, observedHold)),
    ),
  };
}

function getMinimumCameraHoldMs(
  mode: AutoEditMode,
  profile?: AutoEditLearningProfile,
) {
  const modeHold = {
    gentle: 6000,
    balanced: 3500,
  ...
  return Math.round(
    modeHold * (1 - weight) + profile.minimumCameraHoldMs * weight,
  );
}
...
function blendTreatment<T extends Record<string, string | number>>(
  base: T,
  learned: T | undefined,
  weight: number,
): T {
  ...
  return Object.fromEntries(
    Object.entries(base).map(([key, value]) => {
      ...
      if (typeof value === "number" && typeof learnedValue === "number")
        return [
          key,
          Math.round((value * (1 - weight) + learnedValue * weight) * 10) / 10,
        ];
...
function mergeTreatment<T extends Record<string, string | number>>(
  previous: T,
  current: T,
    ...
    Object.entries(current).map(([key, value]) => {
      ...
      if (typeof value === "number" && typeof previousValue === "number")
        return [
          key,
          Math.round(
            ((previousValue * previousSamples + value) /
              (previousSamples + 1)) *
              10,
          ) / 10,
        ];
...
function averageAudioTreatment(
  tracks: TimelineTrack[],
): AutoEditLearningProfile["audioTreatment"] {
  const fallback = {
    audioPreset: "warm" as const,
    noiseReduction: 24,
    noiseGateDb: -54,
  ...
  return {
    audioPreset:
      mostCommon(tracks.map((track) => track.audioPreset)) ??
      fallback.audioPreset,
...
function averageCameraTreatment(
  tracks: TimelineTrack[],
): AutoEditLearningProfile["cameraTreatment"] {
  const fallback = {
  ...
  return {
    cropMode:
      mostCommon(tracks.map((track) => track.cropMode)) ?? fallback.cropMode,
...
function createChapters(
  ...
  const hasSponsor = draft.markers.some((marker) =>
    marker.label.toLowerCase().includes("sponsor"),
  );
  return hasSponsor
    ? [
        ...base.slice(0, 3),
...
function createClipSuggestions(
  ...
  const markerClips = draft.markers
    .slice(0, mode === "clip-hunter" ? 4 : 2)
    .map((marker, index) => ({
