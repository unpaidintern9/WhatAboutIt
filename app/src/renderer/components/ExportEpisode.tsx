...
> = {
  standard: {
    title: "Standard",
    detail: "720p video, 48 kHz audio - quickest shareable copy",
  },
  high: {
    title: "High",
    detail: "1080p video, 320 kbps audio - best finished episode",
    badge: "Recommended",
  },
  archive: {
    title: "Archive",
    detail: "1080p near-master with 24-bit PCM audio - largest file",
  },
};
...
const socialQualityCopy: typeof qualityCopy = {
  standard: {
    title: "Standard",
    detail: "1080×1920 vertical video - quickest social copy",
  },
  high: {
    title: "High",
    detail: "1080×1920 vertical video - cleaner motion and detail",
    badge: "Recommended",
  },
  archive: {
    title: "Archive",
    detail: "1080×1920 vertical video - maximum quality, largest file",
  },
};

function exportEta(job?: ExportJob) {
  if (
    !job ||
  ...
  const remainingSeconds = Math.max(
    1,
    Math.round((elapsedSeconds / job.progress) * (100 - job.progress)),
  );
    ...
    <section className="export-screen">
      <div className="export-hero">
        <div>
          ...
          <p className="soft-copy">
            Your original recording stays safe. Pick the finished copy you need
            and save it locally.
          </p>
        </div>
      ...
      <section className="export-settings-panel">
        <div>
          ...
          <p className="soft-copy">
            {isEditorHandoff
              ? "Build one organized folder an outside editor can open in nearly any modern editing program."
              : "High is the best finished episode. Extra source masters are optional, so a normal export stays fast and compact."}
          </p>
        </div>
        {isEditorHandoff ? (
          <div className="editor-handoff-settings">
            <div className="editor-handoff-summary">
              ...
              <span>
                Reference edit, captions, markers, sync map, README, and SHA-256
                checksums
              </span>
            </div>
            <div className="editor-handoff-destination">
              <span>
                ...
                <small>
                  {destinationFolderPath ??
                    "Choose an external drive, shared folder, or transfer folder."}
                </small>
              </span>
              <Button
                variant={destinationFolderPath ? "secondary" : "primary"}
        ...
        {selectedType === "social-clip-placeholder" ? (
          <p
            className={
              socialClipNeedsRange
                ? "media-tools-status needs-setup"
                : "media-tools-status ready"
            }
            role="status"
          >
            {socialClipNeedsRange
              ? "Go back to Review and drag across the Program timeline to choose the clip."
              : `Selected clip: ${(selectedRangeMs! / 1000).toFixed(1)} seconds · 1080×1920 vertical video`}
          </p>
        ) : null}
        {!isEditorHandoff ? (
          <div className="quality-preset-row" aria-label="Quality preset">
            {(["standard", "high", "archive"] as ExportQualityPreset[]).map(
              (preset) => {
                const copy =
                  selectedType === "social-clip-placeholder"
                    ? socialQualityCopy[preset]
                    : qualityCopy[preset];
                return (
                  <button
                    type="button"
                  ...
                  >
                    <strong>
                      {copy.title}
                      {copy.badge ? <small>{copy.badge}</small> : null}
                    </strong>
        ...
        {!isEditorHandoff ? (
          <button
            type="button"
            className="export-quick-preview"
            disabled={isRunning}
            onClick={() => {
              onQualityChange("standard");
              onMasteringModeChange?.("fast");
            }}
          >
            Quick preview setup · 720p with fast loudness
          </button>
        ) : null}
        {!isEditorHandoff &&
          (selectedType === "full-episode-video" ||
            selectedType === "archive-master") && (
            <div
              className="export-extra-options"
              aria-label="Additional export files"
            >
              {selectedType === "full-episode-video" && (
                <label>
                  <input
                    type="checkbox"
                    checked={includeCameraMasters}
                    onChange={(event) =>
                      onCameraMastersChange?.(event.target.checked)
                    }
                  />
                  <span>
                    <strong>Camera masters</strong>
                    <small>
                      Separate edited video for every camera and its routed
                      microphone. Slowest and largest.
                    </small>
                  </span>
                </label>
              )}
              <label>
                <input
                  type="checkbox"
                  checked={includeAudioMasters}
                  onChange={(event) =>
                    onAudioMastersChange?.(event.target.checked)
                  }
                />
                <span>
                  <strong>24-bit audio masters</strong>
                  <small>
                    Separate WAV files for later remixing or archiving.
                  </small>
                </span>
              </label>
            </div>
        ...
        {!isEditorHandoff ? (
          <div className="export-mastering-choice">
            <span>
              <strong>Podcast loudness</strong>
              <small>
                Measured mastering analyzes the complete mix before applying the
                final level.
              </small>
            </span>
            <div
              className="inspector-segmented"
              aria-label="Podcast loudness processing"
            >
              <button
                type="button"
                className={masteringMode === "measured" ? "selected" : ""}
                onClick={() => onMasteringModeChange?.("measured")}
              >
                Measured
              </button>
              <button
                type="button"
                className={masteringMode === "fast" ? "selected" : ""}
                onClick={() => onMasteringModeChange?.("fast")}
              >
                Fast
              </button>
            </div>
      ...
      <section className="export-progress-panel">
          ...
          <h3>
            {isComplete
              ...
              : isRunning
                ...
                : isError
                  ? "Something needs attention before export"
                  : "Ready to export"}
          </h3>
          {isComplete ? (
        ...
        <p
          className={`media-tools-status ${mediaToolsStatus?.ready ? "ready" : "needs-setup"}`}
        >
        ...
        <div className="export-live-status" role="status" aria-live="polite">
          <strong>
            {isRunning ? `${job?.progress ?? 0}%` : isComplete ? "100%" : ""}
          </strong>
          <span>
            {job?.error
              ? exportFriendlyErrorCopy[job.error]
              : `${job?.message ?? "Save a finished copy"}${eta ? ` · ${eta}` : ""}`}
          </span>
        </div>
        {isComplete &&
          job?.outputFileNames &&
          job.outputFileNames.length > 1 && (
            <div className="export-output-summary">
              <strong>Your export includes</strong>
              {job.outputFileNames.map((fileName) => (
        ...
        <div className="export-actions">
          <Button
            ...
            icon={
              isEditorHandoff ? <PackageOpen size={20} /> : <Play size={20} />
            }
            disabled={
              isRunning || socialClipNeedsRange || handoffNeedsDestination
            }
