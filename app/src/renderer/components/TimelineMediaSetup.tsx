...
export const TimelineMediaSetup = memo(function TimelineMediaSetup({
  media,
  importProgress,
  preparationProgress,
  onImportMedia,
  ...
  onGetEpisodeStorage,
  onCleanupEpisodeStorage,
}: TimelineMediaSetupProps) {
  const [message, setMessage] = useState(
    "Add up to three camera files and the main podcast audio. Full-quality originals stay protected while lighter copies keep editing responsive.",
  );
  ...
  async function runMediaAction(
    ...
    failureLabel: string,
    slot: ReviewMediaImportSlot | "sync",
  ) {
    setBusy(slot);
    try {
  ...
  async function cleanupStorage(scope: EpisodeCleanupScope) {
    ...
    const description =
      scope === "review-cache"
        ? "rebuildable review previews"
        : "finished exports";
    if (
      !window.confirm(
        `Delete this episode's ${description}? Protected originals and editing media will not be touched.`,
      )
    )
      return;
    setStorageBusy(true);
    try {
      ...
      setMessage(
        scope === "review-cache"
          ? "Review cache cleared. Reopen the episode when you want its editing previews rebuilt."
          : "Finished exports deleted. Protected originals were not changed.",
      );
    } catch (error) {
    ...
    <section className="editor-media-setup" aria-label="Episode media setup">
      <div className="editor-media-setup-heading">
        ...
        <div className="editor-media-setup-actions">
          <button
            type="button"
            disabled={!onAutoSync || Boolean(busy)}
            onClick={() =>
              onAutoSync &&
              void runMediaAction(onAutoSync, "Automatic sync failed", "sync")
            }
          >
            <Waves size={17} />{" "}
            {busy === "sync" ? "Working…" : "Sync automatically"}
          </button>
          <button
            type="button"
            disabled={!onVerifyOriginals || Boolean(busy)}
            onClick={() =>
              onVerifyOriginals &&
              void runMediaAction(
                onVerifyOriginals,
      ...
      {preparationProgress && preparationProgress.progress < 100 ? (
        <div
          className="editor-import-progress editor-preparation-progress"
          role="status"
          aria-live="polite"
        >
          <progress
            max="100"
            value={preparationProgress.progress}
            aria-label="Episode media preparation progress"
          />
          <span>
            {preparationProgress.progress}% · {preparationProgress.message}
          </span>
        </div>
      ) : null}
      <div className="editor-media-slots">
        {(
          [
            ...
            "camera-3",
            "morgan-mic",
          ] as ReviewMediaImportSlot[]
        ).map((slot) => {
          const asset = slot.startsWith("camera-")
            ? media?.cameras.find((candidate) => candidate.id === slot)
            : media?.audio.find((candidate) => candidate.id === slot);
          const label = slot.startsWith("camera-")
            ? slot.replace("camera-", "Camera ")
            : "Main audio";
          return (
            <div
              className={asset?.status === "ready" ? "ready" : "missing"}
              key={slot}
            >
              <span>
                {slot.startsWith("camera-") ? (
                  <Video size={17} />
                ) : (
                  <Waves size={17} />
                )}
              ...
              <small>
                {asset?.status === "ready"
                  ? `${asset.codecSummary ?? "Ready"}${asset.durationMs ? ` · ${formatRecordingTime(asset.durationMs)}` : ""}`
                  : "Choose a file"}
              </small>
              <button
                type="button"
                disabled={!onImportMedia || Boolean(busy)}
                onClick={() =>
                  onImportMedia &&
                  void runMediaAction(
                    () => onImportMedia(slot),
              ...
              {asset?.status === "ready" && onRelinkMedia && (
                <button
                  type="button"
                  className="editor-relink-button"
              ...
              {busy === slot && importProgress?.slot === slot && (
                <div className="editor-import-progress">
                  ...
                  {onCancelImport && (
                    <button
                      type="button"
                      className="editor-import-cancel"
                      onClick={() => void onCancelImport(slot)}
                    >
                      Cancel
                    </button>
                  )}
      ...
      {onGetEpisodeStorage && (
        <details
          className="editor-storage-summary"
          onToggle={(event) => {
            if (event.currentTarget.open && !storage && !storageBusy)
              void refreshStorage();
          }}
        >
          <summary>
            Episode media storage
            {storage
          ...
          <div className="editor-storage-actions">
            ...
            <button
              type="button"
              disabled={!onCleanupEpisodeStorage || storageBusy}
              onClick={() => void cleanupStorage("review-cache")}
            >
              Clear review cache
            </button>
            <button
              type="button"
              disabled={!onCleanupEpisodeStorage || storageBusy}
              onClick={() => void cleanupStorage("exports")}
            >
              Delete exports
            </button>
          </div>
