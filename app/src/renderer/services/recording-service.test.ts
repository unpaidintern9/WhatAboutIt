...
function installStudioMock() {
  window.studio = {
    ...
    saveProgramRecording: vi.fn(
      async () => "C:/recording/episode-a/Program/program.webm",
    ),
    saveRecordedTracks: vi.fn(
      async (_folderPath, tracks: RecordingTrackSaveInput[]) =>
        tracks.map((track) => ({
          slot: track.slot,
          kind: track.kind,
          status: "saved" as const,
          filePath: `C:/recording/episode-a/${track.slot}`,
          message: "Saved",
        })),
    ),
    ...
    getMediaToolsStatus: vi.fn(async () => ({
      ready: true,
      message: "Media tools are ready" as const,
    })),
    cancelExport: vi.fn(),
    openExportFolder: vi.fn(),
    createDiagnosticsBundle: vi.fn(async () => ({
      folderPath: "diagnostics",
      files: [],
    })),
    getStorageStatus: vi.fn(async () => ({
      message: "Storage check ready" as const,
      availableBytes: 1024,
    })),
  };
}
...
describe("RecordingService", () => {
  ...
  it("reports the active program and source recorder health", async () => {
    ...
    expect(snapshot.localSaveMessage).toBe(
      "Program plus 4 source tracks are starting their disk writers",
    );
  });
  ...
  it("saves separate recorder tracks after the Program recording", async () => {
    ...
    expect(window.studio.saveRecordedTracks).toHaveBeenCalledWith(
      "C:/recording/episode-a",
      [
        {
          slot: "camera2",
    ...
    expect(snapshot.trackStatuses).toEqual([
      {
        ...
        kind: "camera",
        status: "saved",
        filePath: "C:/recording/episode-a/camera2",
        message: "Saved",
      },
      {
        slot: "guestMic",
        kind: "audio",
        status: "saved",
        filePath: "C:/recording/episode-a/guestMic",
        message: "Saved",
      },
    ]);
  });
  ...
  it("marks disk media recoverable when the recorder cannot finish stopping", async () => {
    installStudioMock();
    const plugin: RecordingEnginePlugin = {
      ...createPlugin(),
      stop: vi.fn(async () => {
    ...
    expect(snapshot).toMatchObject({
      status: "error",
      friendlyError: expect.stringContaining("available for recovery"),
    });
    expect(plugin.shutdown).toHaveBeenCalledTimes(1);
    expect(window.studio.appendRecordingError).toHaveBeenCalledWith(
      "C:/recording/episode-a",
