...
export async function loadReviewMedia(
  episodeId: string,
  options: {
    onProgress?: (progress: ReviewMediaPreparationProgress) => void;
  } = {},
): Promise<ReviewMediaInventory> {
  ...
  report(2, "Finding saved cameras and microphones");
  ...
  const rawProgram =
    assets.find((asset) => asset.kind === "program") ??
    missingAsset(episodeFolder, expectedAssets[0]);
  ...
  const rawCameras = assets
    ...
    .map((asset, index) => {
      ...
      const microphoneSlot =
        cameraMicrophones[cameraSlot] ?? fallbackCameraMicrophones[cameraSlot];
      return {
        ...asset,
  ...
  const rawAudio = assets.filter((asset) => asset.kind === "audio");
  const jobs: Array<{
    kind: "audio" | "program" | "camera";
    asset: ReviewMediaAsset;
  }> = [
    ...rawAudio.map((asset) => ({ kind: "audio" as const, asset })),
    { kind: "program", asset: rawProgram },
    ...rawCameras.map((asset) => ({ kind: "camera" as const, asset })),
  ];
  let completed = 0;
  const prepared = await mapWithConcurrency(jobs, 2, async (job) => {
    let result: ReviewMediaAsset;
    if (job.kind === "audio") {
      result = await ensureMediaWaveform(episodeFolder, job.asset);
    } else {
      const pairedAudio = job.asset.pairedAudioId
        ? rawAudio.find((asset) => asset.id === job.asset.pairedAudioId)
        : undefined;
      result = await ensureMediaWaveform(
        episodeFolder,
        await ensureReviewProxy(episodeFolder, job.asset, pairedAudio),
      );
    }
    completed += 1;
    report(
      5 + (completed / Math.max(1, jobs.length)) * 92,
      `Prepared ${job.asset.label}`,
    );
    return { ...job, asset: result };
  });
  const audio = prepared
    .filter((job) => job.kind === "audio")
    .map((job) => job.asset);
  const program =
    prepared.find((job) => job.kind === "program")?.asset ?? rawProgram;
  const cameras = prepared
    .filter((job) => job.kind === "camera")
    .map((job) => job.asset);
  ...
  const inventory = {
    ...
    audio,
    hasPlayableProgram,
    message: hasPlayableProgram
      ? "Review your recording"
      : "No program video found yet",
  };
  report(100, "Episode media is ready to edit");
  return inventory;
}
...
async function ensureMediaWaveform(
  ...
  try {
    ...
    if (await proxyNeedsRefresh(waveformPath, [asset.filePath])) {
      const width = Math.max(
        1400,
        Math.min(12000, Math.round(((asset.durationMs ?? 0) / 1000) * 2)),
      );
      await runFfmpeg([
        "-y",
        "-i",
        asset.filePath,
        "-filter_complex",
        `aformat=channel_layouts=mono,showwavespic=s=${width}x120:colors=white`,
        "-frames:v",
        "1",
        waveformPath,
      ]);
    ...
    await logger.warning(
      ...
      { filePath: asset.filePath, error: String(error) },
    );
    return asset;
  }
}
...
const importTargets: Record<
  ReviewMediaImportSlot,
  { relativePath: string; kind: "video" | "audio"; label: string }
> = {
  "camera-1": {
    relativePath: path.join("Cameras", "camera-1.webm"),
    kind: "video",
...
export async function importReviewMediaFile(
  ...
  options: {
    signal?: AbortSignal;
    onProgress?: (progress: ReviewMediaImportProgress) => void;
  } = {},
) {
  ...
  const sourceExtension = /^\.[a-z0-9]{1,8}$/i.test(
  ...
  const originalPath = path.join(
    episodeFolder,
    "Originals",
    `${slot}${sourceExtension}`,
  );
  const temporaryOriginalPath = `${originalPath}.importing`;
  await fs.mkdir(path.dirname(targetPath), { recursive: true });
  await fs.mkdir(path.dirname(originalPath), { recursive: true });
  await assertImportDiskSpace(sourceFilePath, episodeFolder, [
    originalPath,
    targetPath,
  ]);
  const report = (progress: number, message: string) =>
  ...
  try {
    ...
    if (
      !(await validatePlayableMedia(
        ...
        undefined,
        target.kind === "video" ? { video: true } : { audio: true },
      ))
    ) {
      throw new Error(`${target.label} original could not be decoded.`);
    }
    assertNotCanceled();
    const durationMs = await getMediaDurationMs(temporaryOriginalPath).catch(
      () => 0,
    );
    if (target.kind === "video") {
      await runFfmpegWithProgress(
        ...
        ],
        {
          ...
          onProgress: (progress) =>
            report(25 + progress * 0.6, "Building a responsive editing copy"),
        },
      );
    } else {
      await runFfmpegWithProgress(
    ...
    await backupExistingImportedMedia(
      episodeFolder,
      originalPath,
      `${slot}-original`,
    );
    ...
    if (slot === "camera-1") {
      ...
      const fallbackMarkerPath = path.join(
        episodeFolder,
        "Session",
      ...
      if (shouldRefreshProgram) {
        ...
        await fs.writeFile(
          fallbackMarkerPath,
          JSON.stringify(
            { source: "camera-1", updatedAt: new Date().toISOString() },
            null,
            2,
          ),
          "utf8",
...
export async function loadImportedOriginalPaths(
  episodeId: string,
): Promise<Partial<Record<ReviewMediaImportSlot, string>>> {
  const episodeFolder = path.join(getEpisodesRoot(), episodeId);
  try {
    const manifest = JSON.parse(
      await fs.readFile(
        path.join(episodeFolder, "Session", "imported-media.json"),
        "utf8",
      ),
    ) as ImportedMediaManifest;
    const resolved: Partial<Record<ReviewMediaImportSlot, string>> = {};
    for (const [slot, asset] of Object.entries(manifest.assets) as Array<
      [ReviewMediaImportSlot, { relativePath: string }]
    >) {
      const candidate = path.resolve(episodeFolder, asset.relativePath);
      if (
        candidate !== episodeFolder &&
        candidate.startsWith(`${episodeFolder}${path.sep}`)
      ) {
        try {
...
async function saveImportedOriginalPath(
  episodeId: string,
  slot: ReviewMediaImportSlot,
  originalPath: string,
) {
...
export async function verifyImportedMediaIntegrity(
  episodeId: string,
  ...
  try {
    ...
    return {
      items: [],
      message: "No protected imported originals have been indexed yet.",
    };
  }
  const items = await Promise.all(
    (
      Object.entries(manifest.assets) as Array<
        [
          ReviewMediaImportSlot,
          NonNullable<ImportedMediaManifest["assets"][ReviewMediaImportSlot]>,
        ]
      >
    ).map(async ([slot, asset]) => {
      ...
      try {
        ...
        if (!asset.sha256)
          return {
            ...
            message:
              "Original found. Relink it once to add an integrity fingerprint.",
          };
        if (asset.sizeBytes !== undefined && stat.size !== asset.sizeBytes)
          return {
            slot,
            status: "changed" as const,
            message: "Original size changed after import.",
          };
        ...
        return matches
          ? {
              ...
              message: "Original matches its protected SHA-256 fingerprint.",
            }
          : {
              slot,
              status: "changed" as const,
              message: "Original contents changed after import.",
            };
      } catch {
        return {
          slot,
          status: "missing" as const,
          message: "Original is missing. Use Relink original to restore it.",
        };
      }
    }),
  );
  ...
  return {
    ...
    message:
      problems === 0
        ? `Verified ${items.length} protected original${items.length === 1 ? "" : "s"}.`
        : `${problems} original${problems === 1 ? " needs" : "s need"} attention.`,
  };
}

export async function relinkImportedMediaFile(
  episodeId: string,
  slot: ReviewMediaImportSlot,
  sourceFilePath: string,
) {
  ...
  if (
    targetPath === episodeFolder ||
    !targetPath.startsWith(`${episodeFolder}${path.sep}`)
  )
    throw new Error("The saved original path is invalid.");
  const target = importTargets[slot];
  if (
    !(await validatePlayableMedia(
      sourceFilePath,
    ...
    throw new Error("The selected file is not playable media.");
  const sourceHash = await calculateFileSha256(sourceFilePath);
  if (expected.sha256 && sourceHash !== expected.sha256)
    throw new Error(
      "That file does not match the original imported recording.",
    );
...
async function assertImportDiskSpace(
  sourceFilePath: string,
  ...
  try {
    ...
    const requiredBytes =
      Math.ceil(sourceBytes * 1.75) + backupBytes + 512 * 1024 * 1024;
    if (availableBytes < requiredBytes) {
      throw new Error(
        `There is not enough free disk space to import this file safely. Free at least ${formatBytes(requiredBytes - availableBytes)} and try again.`,
      );
    }
  } catch (error) {
    if (
      error instanceof Error &&
      error.message.startsWith("There is not enough free disk space")
    )
      throw error;
    await logger.warning(
      "ReviewMedia",
      "Could not complete the import disk-space preflight.",
      { error: String(error) },
    );
  }
}
...
export async function backupExistingImportedMedia(
  episodeFolder: string,
  filePath: string,
  label: string,
) {
  ...
  const backupPath = path.join(
    backupFolder,
    `${prefix}${timestamp}-${randomUUID()}${extension}`,
  );
  try {
    await fs.copyFile(filePath, backupPath);
  } catch (error) {
    throw new Error(
      `The existing ${label} could not be backed up, so the import was stopped before anything was replaced.`,
      { cause: error },
    );
  }
  const matchingBackups = (
    await fs.readdir(backupFolder, { withFileTypes: true })
  )
...
export async function analyzeReviewMediaSync(
  episodeId: string,
): Promise<ReviewMediaSyncResult> {
  const episodeFolder = path.join(getEpisodesRoot(), episodeId);
  const referenceCandidates = [
    path.join(episodeFolder, "Audio", "morgan-mic.m4a"),
  ...
  const referenceOnsets =
    referenceEnvelope.length === 0
      ? await detectSoundOnsetsMs(referencePath)
      : [];
  if (referenceEnvelope.length === 0 && referenceOnsets.length === 0)
    return {
      ...
      confidence: "review",
      message:
        "Automatic sync could not find a clear sound in the main audio. Use Sync nudge to line up a clap or spoken word.",
    };
  ...
  for (const index of [1, 2, 3]) {
    ...
    try {
      ...
      const cameraEnvelope =
        referenceEnvelope.length > 0
          ? await extractWaveformEnvelope(cameraPath).catch(() => [])
          : [];
      const alignment =
        cameraEnvelope.length > 0
          ? correlateAudioEnvelopes(referenceEnvelope, cameraEnvelope)
          : alignSoundOnsets(
              referenceOnsets.length > 0
                ? referenceOnsets
                : await detectSoundOnsetsMs(referencePath),
              await detectSoundOnsetsMs(cameraPath),
            );
  ...
  const confidence =
    count > 0 && cameraConfidence.every((value) => value === "high")
      ? "high"
      : "review";
  return {
    offsetsMs,
...
async function extractWaveformEnvelope(filePath: string) {
  ...
  try {
    ...
    for (
      let byteOffset = 0;
      byteOffset + samplesPerWindow * 2 <= pcm.length;
      byteOffset += samplesPerWindow * 2
    ) {
      let energy = 0;
...
export function correlateAudioEnvelopes(
  reference: number[],
  camera: number[],
  intervalMs = SYNC_ENVELOPE_INTERVAL_MS,
) {
  const maxLag = Math.round(30000 / intervalMs);
  const minimumOverlap = Math.min(
    reference.length,
    camera.length,
    Math.round(5000 / intervalMs),
  );
  ...
  const separatedRunnerUp = scores.find(
    (candidate) =>
      Math.abs(candidate.lag - best.lag) >= Math.round(200 / intervalMs),
  );
  const margin = best.score - (separatedRunnerUp?.score ?? 0);
  return {
    offsetMs: Math.max(
      -30000,
      Math.min(30000, Math.round(best.lag * intervalMs)),
    ),
    confidence:
      best.score >= 0.5 && margin >= 0.05
        ? ("high" as const)
        : ("review" as const),
...
export function alignSoundOnsets(
  referenceOnsetsMs: number[],
  cameraOnsetsMs: number[],
) {
  const candidates = referenceOnsetsMs.flatMap((reference) =>
    cameraOnsetsMs
      .map((camera) => camera - reference)
      .filter((offset) => Math.abs(offset) <= 30000),
  );
  if (candidates.length === 0) return undefined;
  const scored = candidates
    .map((candidate) => {
      const possibleMatches = referenceOnsetsMs
        ...
        .filter((match) => match.errorMs <= 250)
      ...
      const matches = possibleMatches.filter((match) => {
        if (
          usedReferences.has(match.referenceIndex) ||
          usedCameras.has(match.cameraIndex)
        )
          return false;
      ...
      const errors = differences.map((difference) =>
        Math.abs(difference - candidate),
      );
      const matchedReferenceTimes = matches.map(
        (match) => referenceOnsetsMs[match.referenceIndex],
      );
      return {
        candidate,
        differences,
        ...
        matchSpanMs:
          matchedReferenceTimes.length > 1
            ? Math.max(...matchedReferenceTimes) -
              Math.min(...matchedReferenceTimes)
            : 0,
        averageErrorMs:
          errors.length > 0
        ...
        maxErrorMs:
          errors.length > 0 ? Math.max(...errors) : Number.POSITIVE_INFINITY,
      };
    })
    .sort(
      (left, right) =>
        right.matchCount - left.matchCount ||
        left.averageErrorMs - right.averageErrorMs,
    );
  const best = scored[0];
  const orderedDifferences = [...best.differences].sort(
    (left, right) => left - right,
  );
  const offsetMs = Math.round(
    orderedDifferences[Math.floor(orderedDifferences.length / 2)] ??
      best.candidate,
  );
  return {
    offsetMs: Math.max(-30000, Math.min(30000, offsetMs)),
    confidence:
      best.matchCount >= 3 && best.matchSpanMs >= 1000 && best.maxErrorMs <= 120
        ? ("high" as const)
        : ("review" as const),
...
async function ensureReviewProxy(
  ...
  const usablePairedAudio =
    pairedAudio?.status === "ready" && pairedAudio.filePath
      ? pairedAudio
      : undefined;
  ...
  try {
    ...
    if (!(await validatePlayableMedia(proxyPath, undefined, requirements)))
      throw new Error("Review proxy validation failed.");
    ...
    return {
      ...
      durationMs:
        Number.isFinite(duration) && duration > 0
          ...
          : asset.durationMs,
      message: usablePairedAudio
        ? `Ready with ${asset.pairedAudioLabel}`
        : asset.message,
    };
  } catch (error) {
    ...
    await logger.warning(
      "ReviewMedia",
      "Review proxy could not be prepared; using the original file.",
      { filePath: sourceFile, error: String(error) },
    );
    return asset;
  }
}
...
async function proxyNeedsRefresh(proxyPath: string, sourcePaths: string[]) {
  try {
    const [proxyStat, ...sourceStats] = await Promise.all([
      fs.stat(proxyPath),
      ...sourcePaths.map((sourcePath) => fs.stat(sourcePath)),
    ]);
    return sourceStats.some(
      (sourceStat) => sourceStat.mtimeMs > proxyStat.mtimeMs,
    );
  } catch {
    return true;
  }
...
async function inspectAsset(
  ...
  try {
    ...
    try {
      ...
      await logger.warning(
        "ReviewMedia",
        "Media exists but could not be probed.",
...
async function loadRecordingDuration(episodeFolder: string) {
  try {
    ...
    ) as RecordingStateFile;
    return typeof state.elapsedMs === "number" &&
