...
import {
  detectHardwareVideoEncoder,
  detectMediaTools,
  getMediaDurationMs,
  requireMediaTools,
  runFfmpeg,
  runFfmpegWithProgress,
  validatePlayableMedia,
  type HardwareVideoEncoder,
} from "./ffmpeg-tools";
import { logger } from "./logger";
import {
  loadImportedOriginalPaths,
  mediaFilePlaybackUrl,
} from "./review-media-store";
...
async function requestExportFolder(request: ExportRequest) {
  ...
  if (!request.destinationFolderPath)
    throw new Error("Choose where to save the editor handoff package.");
...
async function writeExportArtifacts(job: ExportJob) {
  ...
  await fs.writeFile(
    path.join(job.outputFolder, "export-job.json"),
...
function outputFileName(type: ExportRequest["type"]) {
  ...
  if (type === "social-clip-placeholder")
    return "what-about-it-social-clip.mp4";
  if (type === "editor-handoff")
    return path.join("01 Reference Edit", "what-about-it-reference-edit.mp4");
  return "what-about-it-full-episode-video.mp4";
}

export async function nextAvailableExportPath(
  folder: string,
  ...
  for (let version = 1; ; version += 1) {
    ...
    if (!(await fileExists(path.join(folder, fileName))))
      return { fileName, outputPath: path.join(folder, fileName) };
  }
}

async function hasEnoughSpaceForExport(
  folder: string,
  sourceFile: string,
  ...
  try {
    ...
    const cameraMasterEstimate =
      request.type === "full-episode-video" &&
      !request.practice &&
      request.includeCameraMasters !== false
        ? sourceBytes * 3
        : 0;
    const audioMasterEstimate =
      (request.type === "full-episode-video" ||
        request.type === "archive-master") &&
      !request.practice &&
      request.includeAudioMasters !== false
        ? sourceBytes
        : 0;
    const requiredBytes = Math.ceil(
      primaryEstimate +
        cameraMasterEstimate +
        audioMasterEstimate +
        1024 * 1024 * 1024,
    );
    ...
    await logger.warning(
      "ExportService",
      "Could not complete the export disk-space preflight.",
      { error: String(error) },
    );
    return true;
  }
...
function qualityArgs(
  ...
  preset: ExportRequest["qualityPreset"],
  includeVideoFilter = true,
  hardwareEncoder?: HardwareVideoEncoder,
) {
  if (type === "audio-only") {
    return [
      "-vn",
      ...
      "aac",
      "-b:a",
      preset === "high" || preset === "archive" ? "320k" : "192k",
    ];
  }
    ...
    type === "editor-handoff"
      ...
      : preset === "high"
        ...
        : preset === "archive"
          ...
          : "20";
  const speed =
    type === "editor-handoff" || preset === "high" || preset === "archive"
      ? "slow"
      : "veryfast";
  const audioBitrate =
    type === "editor-handoff" || preset !== "standard" ? "320k" : "192k";
  const hardwareArgs =
      ...
      : hardwareEncoder === "h264_qsv"
        ...
        : hardwareEncoder === "h264_amf"
          ? [
              ...
              hardwareEncoder,
              "-quality",
              preset === "standard" ? "speed" : "balanced",
              "-qp_i",
              crf,
              "-qp_p",
              crf,
...
function outputSize(
  ...
  return preset === "high" || preset === "archive"
    ? { width: 1920, height: 1080 }
    : { width: 1280, height: 720 };
}

function videoOutputFilter(
  preset: ExportRequest["qualityPreset"],
  type?: ExportRequest["type"],
) {
...
async function renderExport(input: {
  ...
  const mapArgs =
    input.request.type === "audio-only"
      ? ["-map", "0:a:0"]
      : input.audioFile
        ? ["-map", "0:v:0", "-map", "1:a:0"]
        : ["-map", "0:v:0", "-map", "0:a:0"];
  const containerArgs = input.outputPath.endsWith(".mp4")
    ? ["-movflags", "+faststart"]
    : [];
  ...
  await runFfmpegWithProgress(
    [
      ...
      ...qualityArgs(
        input.request.type,
        input.request.qualityPreset,
        true,
        input.hardwareEncoder,
      ),
      ...containerArgs,
      input.outputPath,
    ],
...
async function renderDraftExport(input: {
  ...
  for (const track of request.draft.tracks) {
    ...
    const filePath = sourcePathForTrack(
      request.episodeId,
      track,
      originalSources,
    );
    if (!filePath || !(await fileExists(filePath))) continue;
    if (track.kind === "camera" && track.includedInProgram)
      cameraInputs.push({ track, filePath, inputIndex });
    if (track.kind === "microphone")
      audioInputs.push({ track, filePath, inputIndex });
    if (track.kind !== "camera" && track.kind !== "microphone") continue;
    inputArgs.push("-i", filePath);
    inputIndex += 1;
  }

  const hasDraftDecisions =
    request.draft.cameraDecisions.length > 0 ||
  ...
  const selectedClipRange =
    request.type === "social-clip-placeholder" &&
    request.draft.selection?.endTimestampMs !== undefined
      ? {
          startMs: request.draft.selection.timestampMs,
          endMs: request.draft.selection.endTimestampMs,
        }
      : undefined;
  const globalEdits = [
    ...
    ...(selectedClipRange
      ? [
          {
            id: "social-clip-in",
            type: "trim-before" as const,
            label: "Social clip start",
            timestampMs: selectedClipRange.startMs,
            targetTrackId: "program",
            createdAt: request.draft.updatedAt,
          },
          {
            id: "social-clip-out",
            type: "trim-after" as const,
            label: "Social clip end",
            timestampMs: selectedClipRange.endMs,
            targetTrackId: "program",
            createdAt: request.draft.updatedAt,
          },
        ]
  ...
  const cameraEditPoints = request.draft.editLog.flatMap((edit) => {
    const target = request.draft.tracks.find(
      (track) => track.id === edit.targetTrackId,
    );
  ...
  const keepRanges = createVideoRanges(
    ...
    globalEdits,
    [
      ...request.draft.cameraDecisions.map((decision) => decision.startMs),
      ...cameraEditPoints,
    ],
  );
  const editedDurationMs = keepRanges.reduce(
    (total, range) => total + range.endMs - range.startMs,
    0,
  );
  const cameraByTrack = new Map(
    cameraInputs.map((item) => [item.track.id, item]),
  );
  const programTrack = request.draft.tracks.find(
    (track) => track.id === "program",
  );
  const size = outputSize(request.qualityPreset, request.type);

  if (request.type !== "audio-only") {
    const segments = keepRanges.map((range) => {
      const decision = [...request.draft.cameraDecisions]
        .reverse()
        .find((item) => item.startMs <= range.startMs);
      const camera = decision
        ? cameraByTrack.get(decision.cameraTrackId)
        : undefined;
      const midpoint = range.startMs + (range.endMs - range.startMs) / 2;
      const chosenCamera =
        camera &&
        isTimelineTrackAvailableAt(request.draft, camera.track.id, midpoint)
          ? camera
          : undefined;
      ...
      const pictureFilters = createVideoTreatment(
        request.type === "social-clip-placeholder" && chosenTrack
          ? { ...chosenTrack, positionX: 0, positionY: 0 }
          : chosenTrack,
        size,
      );
      return {
    ...
    if (segments.length === 0)
      throw new Error("The draft removed every video section.");

    const transitionInto = segments.map((segment, index) => {
      const previous = segments[index - 1];
      if (
        !previous ||
        request.draft.cameraTransition !== "fade" ||
        previous.range.endMs !== segment.range.startMs
      )
        return 0;
      ...
      return Math.max(
        0,
        Math.min(requested, segment.range.startMs + segment.syncOffsetMs),
      );
    });
    ...
    for (const [index, segment] of segments.entries()) {
      ...
      if (outgoingTransitionMs > 0) {
        ...
        addVideoSlice(
          segment,
          segment.range.endMs - outgoingTransitionMs,
          segment.range.endMs,
          outgoingLabel,
        );
        addVideoSlice(
          next,
          next.range.startMs - outgoingTransitionMs,
          next.range.startMs,
          incomingLabel,
        );
        filters.push(
  ...
  const enabledAudioInputs = audioInputs.filter(
    (item) => item.track.includedInProgram && !item.track.muted,
  );
  const anySolo = enabledAudioInputs.some((item) => item.track.solo);
  const activeAudioInputs = anySolo
    ? enabledAudioInputs.filter((item) => item.track.solo)
    : enabledAudioInputs;
  const allSeparateMicrophonesMuted =
    audioInputs.length > 0 && activeAudioInputs.length === 0;
  const audioSources =
    activeAudioInputs.length > 0
      ? activeAudioInputs.map((item) => ({
      ...
      : [
          {
            inputIndex: 0,
            track: request.draft.tracks.find((track) => track.id === "program"),
          },
        ];
  for (const [index, source] of audioSources.entries()) {
    const label = `drafta${index}`;
    const trackEdits = editsForTrack(
      request.draft.editLog,
      source.track?.id ?? "program",
    );
    const volume = allSeparateMicrophonesMuted
      ? 0
      : Math.max(0, Math.min(1.5, (source.track?.volume ?? 100) / 100));
  ...
  const loudnessTarget = Math.max(
    -24,
    Math.min(-12, request.draft.loudnessTargetLufs ?? -16),
  );
  const truePeak = Math.max(
    -3,
    Math.min(-0.5, request.draft.truePeakDb ?? -1.5),
  );
  const loudnessFinish =
    request.masteringMode === "measured"
      ? "aresample=48000"
...
function sourcePathForTrack(
  episodeId: string,
  track: TimelineTrack,
  originalSources: Awaited<ReturnType<typeof loadImportedOriginalPaths>> = {},
) {
  const original = track.sourceAssetId
    ? originalSources[track.sourceAssetId as keyof typeof originalSources]
    : undefined;
  if (original) return original;
  if (
    track.kind === "camera" &&
    /^camera-[123]$/.test(track.sourceAssetId ?? "")
  ) {
    return path.join(
      episodeFolder(episodeId),
      "Cameras",
      `${track.sourceAssetId}.webm`,
    );
  }
  ...
  const audioFile = track.sourceAssetId
    ? audioFiles[track.sourceAssetId]
    : undefined;
  return audioFile
    ? path.join(episodeFolder(episodeId), "Audio", audioFile)
    : undefined;
}
...
export async function renderTrackTreatmentPreview(input: {
  episodeId: string;
  draft: ExportRequest["draft"];
  trackId: string;
  timestampMs: number;
}): Promise<ReviewMediaTreatmentPreview> {
  const track = input.draft.tracks.find(
    (candidate) => candidate.id === input.trackId,
  );
  if (!track || (track.kind !== "camera" && track.kind !== "microphone"))
    throw new Error(
      "Choose a camera or microphone track to preview its effects.",
    );
  ...
  if (!sourceFile || !(await fileExists(sourceFile)))
    throw new Error(`${track.label} does not have a media file to preview.`);
  const sourceDurationMs = await getMediaDurationMs(sourceFile);
  const startMs = Math.max(
    0,
    Math.min(sourceDurationMs - 500, input.timestampMs - 2000),
  );
  const durationMs = Math.max(500, Math.min(10000, sourceDurationMs - startMs));
  const previewFolder = path.join(
    episodeFolder(input.episodeId),
    "Session",
    "Review",
  );
  const safeTrackId = track.id.replace(/[^a-z0-9-]+/gi, "-");
  const kind =
    track.kind === "camera" ? ("video" as const) : ("audio" as const);
  const previewPrefix = `treatment-preview-${safeTrackId}-`;
  const outputPath = path.join(
    previewFolder,
    `${previewPrefix}${randomUUID()}.${kind === "video" ? "webm" : "m4a"}`,
  );
  ...
  if (kind === "video") {
    ...
    await runFfmpeg([
      ...
      "-an",
      "-vf",
      `${crop}${createVideoTreatment(track, size)}setsar=1,fps=30`,
      "-c:v",
      "libvpx-vp9",
      "-deadline",
      "realtime",
    ...
    const loudnessTarget = Math.max(
      -24,
      Math.min(-12, input.draft.loudnessTargetLufs ?? -16),
    );
    const truePeak = Math.max(
      -3,
      Math.min(-0.5, input.draft.truePeakDb ?? -1.5),
    );
  ...
  if (
    !(await validatePlayableMedia(
      outputPath,
      undefined,
      kind === "video"
        ? { video: true, decode: true }
  ...
  const oldPreviews = (await fs.readdir(previewFolder, { withFileTypes: true }))
    .filter(
      (entry) =>
        entry.isFile() &&
        entry.name.startsWith(previewPrefix) &&
        entry.name !== path.basename(outputPath),
    )
    .sort((left, right) => right.name.localeCompare(left.name));
  for (const oldPreview of oldPreviews.slice(4)) {
    await fs
      .rm(path.join(previewFolder, oldPreview.name), { force: true })
      .catch(() => undefined);
  }
  return {
    trackId: track.id,
...
function createVideoRanges(
  durationMs: number,
  globalEdits: TimelineEditOperation[],
  decisionPoints: number[],
) {
  const trimStart = Math.max(
    0,
    ...globalEdits
      .filter((edit) => edit.type === "trim-before")
      .map((edit) => edit.timestampMs),
  );
  const trimEnd = Math.min(
    durationMs,
    ...globalEdits
      .filter((edit) => edit.type === "trim-after")
      .map((edit) => edit.timestampMs),
    durationMs,
  );
  const cuts = globalEdits
    .filter((edit) => edit.type === "delete-section")
    .map((edit) => ({
      startMs: edit.timestampMs,
      endMs: Math.min(
        durationMs,
        edit.endTimestampMs ?? edit.timestampMs + 15000,
      ),
    }));
  ...
  const ordered = [...boundaries]
    .filter((value) => value >= trimStart && value <= trimEnd)
    .sort((a, b) => a - b);
  return ordered
    .slice(0, -1)
    .map((startMs, index) => ({ startMs, endMs: ordered[index + 1] }))
    .filter(
      (range) =>
        range.endMs > range.startMs &&
        !cuts.some(
          (cut) => range.startMs >= cut.startMs && range.endMs <= cut.endMs,
        ),
    );
}
...
function createMuteExpression(edits: TimelineEditOperation[], volume: number) {
  ...
  for (const edit of edits) {
    if (edit.type === "trim-before")
      muted.push(`lt(t,${seconds(edit.timestampMs)})`);
    if (edit.type === "trim-after")
      muted.push(`gte(t,${seconds(edit.timestampMs)})`);
    if (edit.type === "delete-section")
      muted.push(
        `between(t,${seconds(edit.timestampMs)},${seconds(edit.endTimestampMs ?? edit.timestampMs + 15000)})`,
      );
  }
  return muted.length > 0
    ? `if(${muted.join("+")},0,${volume.toFixed(3)})`
    : volume.toFixed(3);
}
...
function createKeepExpression(edits: TimelineEditOperation[]) {
  ...
  for (const edit of edits) {
    if (edit.type === "trim-before")
      removed.push(`lt(t,${seconds(edit.timestampMs)})`);
    if (edit.type === "trim-after")
      removed.push(`gte(t,${seconds(edit.timestampMs)})`);
    if (edit.type === "delete-section")
      removed.push(
        `between(t,${seconds(edit.timestampMs)},${seconds(edit.endTimestampMs ?? edit.timestampMs + 15000)})`,
      );
...
function createAudioSyncFilter(syncOffsetMs: number) {
  if (syncOffsetMs > 0)
    return `atrim=start=${seconds(syncOffsetMs)},asetpts=PTS-STARTPTS,`;
  if (syncOffsetMs < 0) {
...
function createAudioTreatment(track: TimelineTrack | undefined) {
  ...
  if (preset === "warm")
    filters.push("highpass=f=70", "equalizer=f=180:t=q:w=1:g=1.5");
  if (preset === "broadcast")
    filters.push("highpass=f=80", "equalizer=f=3200:t=q:w=1:g=2");
  if (track) {
    if (track.noiseReduction > 0)
      filters.push(
        `afftdn=nr=${Math.max(0.01, track.noiseReduction * 0.65).toFixed(2)}:nf=-50:tn=1`,
      );
    if (track.noiseGateDb > -80) {
      const threshold = Math.pow(10, track.noiseGateDb / 20);
      filters.push(
        `agate=threshold=${threshold.toFixed(5)}:ratio=2.5:attack=15:release=220:range=0.08`,
      );
    }
    if (track.deEsser > 0)
      filters.push(
        `equalizer=f=6500:t=q:w=1.2:g=${(-track.deEsser * 0.045).toFixed(2)}`,
      );
    if (track.eqLowDb !== 0)
      filters.push(`equalizer=f=120:t=q:w=0.8:g=${track.eqLowDb.toFixed(1)}`);
    if (track.eqMidDb !== 0)
      filters.push(`equalizer=f=1200:t=q:w=1:g=${track.eqMidDb.toFixed(1)}`);
    if (track.eqHighDb !== 0)
      filters.push(`equalizer=f=6000:t=q:w=0.8:g=${track.eqHighDb.toFixed(1)}`);
    if (track.compression > 0) {
      const ratio = 1.5 + track.compression * 0.045;
      const threshold = 0.24 - track.compression * 0.0017;
      filters.push(
        `acompressor=threshold=${Math.max(0.05, threshold).toFixed(3)}:ratio=${ratio.toFixed(2)}:attack=5:release=110:makeup=1.15`,
      );
    }
  }
...
function createVideoTreatment(
  ...
  if (track?.temperature || track?.tint)
    filters.push(
      `colorbalance=rm=${temperature}:bm=${(-Number(temperature)).toFixed(3)}:gm=${tint}`,
    );
  if ((track?.denoise ?? 0) > 0) {
    const strength = ((track?.denoise ?? 0) / 25).toFixed(2);
    filters.push(
      `hqdn3d=${strength}:${strength}:${(Number(strength) * 1.5).toFixed(2)}:${(Number(strength) * 1.5).toFixed(2)}`,
    );
  }
  if ((track?.sharpness ?? 0) > 0)
    filters.push(
      `unsharp=5:5:${((track?.sharpness ?? 0) * 0.015).toFixed(2)}:5:5:0`,
    );
  return `${filters.join(",")},`;
}
...
function createFadeFilter(
  ...
  if (track.fadeOutMs > 0)
    filters.push(
      `afade=t=out:st=${seconds(Math.max(0, durationMs - track.fadeOutMs))}:d=${seconds(track.fadeOutMs)}`,
    );
  return filters.length > 0 ? `,${filters.join(",")}` : "";
}
...
async function renderMeasuredLoudness(input: {
  ...
  const targetI = Math.max(
    -24,
    Math.min(-12, input.request.draft.loudnessTargetLufs ?? -16),
  );
  const targetTp = Math.max(
    -3,
    Math.min(-0.5, input.request.draft.truePeakDb ?? -1.5),
  );
  const analysis = await runFfmpeg([
    ...
    "0:a:0",
    "-af",
    `loudnorm=I=${targetI}:LRA=11:TP=${targetTp}:print_format=json`,
    "-f",
    "null",
    "-",
  ]);
  ...
  if (!matches?.length)
    throw new Error(
      "Measured loudness analysis did not return usable results.",
    );
  ...
  const mapArgs = isAudioOnly
    ? ["-map", "0:a:0"]
    : ["-map", "0:v:0", "-map", "0:a:0", "-c:v", "copy"];
  const audioArgs =
    input.request.type === "archive-master"
      ...
      : [
          ...
          "aac",
          "-b:a",
          input.request.qualityPreset === "standard" ? "192k" : "320k",
        ];
  await runFfmpegWithProgress(
    [
      "-y",
      ...
      filter,
      ...audioArgs,
      ...(input.outputPath.endsWith(".mp4") ? ["-movflags", "+faststart"] : []),
      input.outputPath,
    ],
    {
      durationMs: input.durationMs,
...
async function createCameraMasters(
  ...
  try {
    const deviceMap = JSON.parse(
      await fs.readFile(
        path.join(
          episodeFolder(request.episodeId),
          "Session",
          "device-map.json",
        ),
        "utf8",
  ...
  for (const cameraSlot of [
    ...
    const sourceFile =
      originalSources[
        `camera-${cameraNumber}` as keyof typeof originalSources
      ] ??
      path.join(
        episodeFolder(request.episodeId),
        "Cameras",
        `camera-${cameraNumber}.webm`,
      );
    const microphoneSlot =
      savedRoutes[cameraSlot] ??
      request.deviceDefaults?.cameraMicrophones?.[cameraSlot] ??
      fallbackCameraMicrophones[cameraSlot];
    const audioFile = path.join(
      episodeFolder(request.episodeId),
      "Audio",
      microphoneFileNames[microphoneSlot],
    );
    if (!(await fileExists(sourceFile)) || !(await fileExists(audioFile)))
      continue;
    available.push({
      cameraSlot,
      sourceFile,
      ...
      relativeOutput: path.join(
        "Camera Masters",
        `camera-${cameraNumber}-with-${microphoneFileNames[microphoneSlot].replace(".m4a", "")}.mp4`,
      ),
    });
  }
  ...
  for (const [index, item] of available.entries()) {
    ...
    const reserved = await nextAvailableExportPath(
      exportFolder(request.episodeId),
      item.relativeOutput,
    );
    const outputPath = reserved.outputPath;
    await fs.mkdir(path.dirname(outputPath), { recursive: true });
    const durationMs =
      (await getMediaDurationMs(item.audioFile)) || sourceDurationMs;
    await renderExport({
      ...
      onProgress: (progress) =>
        updateRunningJob(
          running,
          start + (progress / 100) * width,
          `Building Camera ${item.cameraSlot.at(-1)} with its microphone`,
          report,
        ),
    });
    if (
      !(await validatePlayableMedia(outputPath, undefined, {
        video: true,
        audio: true,
        decode: true,
      }))
      ...
      throw new Error(
        `${item.relativeOutput} could not be decoded with video and audio.`,
      );
    }
...
async function createAudioMasters(
  ...
  const outputs: string[] = [];
  const originalSources = await loadImportedOriginalPaths(request.episodeId);
  const tracks = request.draft.tracks.filter(
    (track) => track.kind === "microphone" && track.sourceAssetId,
  );
  const available: Array<{
    track: TimelineTrack;
    sourceFile: string;
  ...
  for (const track of tracks) {
    const sourceFile = sourcePathForTrack(
      request.episodeId,
      track,
      originalSources,
    );
    ...
    available.push({
      ...
      relativeOutput: path.join(
        "Audio Masters",
        `${track.sourceAssetId}-edited.wav`,
      ),
    });
  }
  ...
  for (const [index, item] of available.entries()) {
    ...
    const filters = [
      ...
      createFadeFilter(
        item.track,
        request.draft.durationMs || sourceDurationMs,
      ),
      ...
      ],
      {
        ...
        onProgress: (progress) =>
          updateRunningJob(
            running,
            start + (progress / 100) * width,
            `Building ${item.track.label} audio master`,
            report,
          ),
      },
    );
...
async function createEditorCameraFiles(
  ...
  for (const [index, item] of available.entries()) {
    ...
    await fs.mkdir(path.dirname(outputPath), { recursive: true });
    const durationMs =
      (await getMediaDurationMs(item.sourceFile)) || sourceDurationMs;
    await runFfmpegWithProgress(
      [
        "-y",
        "-fflags",
        ...
        "0:a:0?",
        "-vf",
        videoOutputFilter("high", "editor-handoff"),
        "-r",
        "30",
        "-pix_fmt",
        "yuv420p",
      ...
      ],
      {
        ...
        onProgress: (progress) =>
          updateRunningJob(
            running,
            start + (progress / 100) * width,
            `Preparing Camera ${item.cameraNumber} for outside editors`,
            report,
          ),
      },
    );
...
async function createEditorAudioFiles(
  ...
  const originalSources = await loadImportedOriginalPaths(request.episodeId);
  const labels: Array<{
    assetId: "morgan-mic" | "guest-mic" | "extra-mic";
    label: string;
  }> = [
    ...
    return [
      {
        ...item,
        sourceFile,
        relativeOutput: path.join("03 Isolated Audio", `${item.label}.wav`),
      },
    ];
  });
  const existing = [] as typeof available;
  for (const item of available)
    if (await fileExists(item.sourceFile)) existing.push(item);
...
function createSyncMap(request: ExportRequest) {
  const rows = request.draft.tracks
    ...
    .map((track) => {
      ...
      const action =
        offset > 0
          ? `Trim ${offset} ms from the start`
          : offset < 0
            ? `Delay by ${Math.abs(offset)} ms`
            : "No offset";
      return [
        track.label,
        track.kind,
...
function createMarkerCsv(request: ExportRequest) {
  const rows = request.draft.markers.map((marker) =>
    [
      marker.label,
      marker.timestampMs,
      (marker.timestampMs / 1000).toFixed(3),
      marker.createdAt,
    ]
      .map(csvCell)
      .join(","),
...
function editorReadme(
  request: ExportRequest,
  cameraOutputs: string[],
  audioOutputs: string[],
) {
  return [
...
async function packageFiles(
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory())
      files.push(...(await packageFiles(folder, entryPath)));
    else if (
      entry.isFile() &&
      ![
        "SHA256SUMS.txt",
      ...
      files.push(path.relative(folder, entryPath));
  }
  return files.sort((left, right) => left.localeCompare(right));
}

async function writeEditorHandoffNotes(
  request: ExportRequest,
  running: ExportJob,
  cameraOutputs: string[],
  audioOutputs: string[],
  ...
  await Promise.all([
    fs.writeFile(
      path.join(running.outputFolder, "README - START HERE.txt"),
      editorReadme(request, cameraOutputs, audioOutputs),
      "utf8",
    ),
    fs.writeFile(
      path.join(notesFolder, "sync-map.csv"),
      createSyncMap(request),
      "utf8",
    ),
    fs.writeFile(
      path.join(notesFolder, "markers.csv"),
      createMarkerCsv(request),
      "utf8",
    ),
    copyIfExists(
      path.join(
      ...
      path.join(notesFolder, "recording-sync-metadata.json"),
    ),
    copyIfExists(
      path.join(
        episodeFolder(request.episodeId),
      ...
      path.join(notesFolder, "recording-capture-manifest.json"),
    ),
  ]);
  ...
  for (const relativePath of files) {
    checksums.push(
      `${await sha256File(path.join(running.outputFolder, relativePath))}  ${relativePath.split(path.sep).join("/")}`,
    );
  }
  await fs.writeFile(
    path.join(running.outputFolder, "SHA256SUMS.txt"),
    `${checksums.join("\n")}\n`,
    "utf8",
  );
  return [
...
async function createExportUnlocked(
  request: ExportRequest,
  report?: ExportProgressReporter,
): Promise<ExportJob> {
  ...
  if (!mediaTools.ready) {
    ...
    report?.(failed);
    await logger.warning("ExportService", "Media tools missing for export.", {
      episodeId: request.episodeId,
    });
    return failed;
  }
  ...
  if (
    request.type === "social-clip-placeholder" &&
    (request.draft.selection?.endTimestampMs === undefined ||
      request.draft.selection.endTimestampMs <=
        request.draft.selection.timestampMs)
  ) {
  ...
  const canExport =
    request.practice || (await hasProgramRecording(request.episodeId));
  if (!canExport) {
    ...
    await logger.warning(
      "ExportService",
      "Recording file missing for export.",
      { episodeId: request.episodeId },
    );
    return failed;
  }

  const reservedOutput = await nextAvailableExportPath(
    folder,
    outputFileName(request.type),
  );
  const fileName = reservedOutput.fileName;
  const outputPath = reservedOutput.outputPath;
  const workingOutputPath = path.join(
    path.dirname(outputPath),
    `.${path.basename(outputPath, path.extname(outputPath))}-rendering-${randomUUID()}${path.extname(outputPath)}`,
  );
  await fs.mkdir(path.dirname(outputPath), { recursive: true });
  const tools = await requireMediaTools();
  const sourceFile = request.practice
    ? await createPracticeSource(request.episodeId)
    : await findProgramRecording(request.episodeId);
  if (!sourceFile) {
    const failed = failExportJob(running, "recording-missing");
    await writeExportArtifacts(failed);
    report?.(failed);
  ...
  try {
    ...
    const sourceDurationMs =
      (await getMediaDurationMs(sourceFile, tools)) ||
      request.draft.durationMs ||
      1000;
    if (
      !(await hasEnoughSpaceForExport(
        folder,
        sourceFile,
        request,
        sourceDurationMs,
    ...
    const hardwareEncoder =
      request.type !== "audio-only" && request.type !== "archive-master"
        ? await detectHardwareVideoEncoder(tools)
        : undefined;
    const premasterPath = measuredMastering
      ? path.join(
          folder,
          `.${path.basename(outputPath, path.extname(outputPath))}-premaster-${randomUUID()}${path.extname(outputPath)}`,
        )
      : workingOutputPath;
    const renderInput = {
      request,
      ...
      onProgress: (progress: number) => {
        running = updateRunningJob(
          running,
          12 + (progress / 100) * (measuredMastering ? 30 : 48),
    ...
    try {
      const renderPrimary = async () => {
        renderedDraft = Boolean(
          !request.practice && (await renderDraftExport(renderInput)),
        );
      ...
      try {
        ...
        if (!renderInput.hardwareEncoder || controller.signal.aborted)
          throw error;
        await logger.warning(
          "ExportService",
          "Hardware video encoding was unavailable during the real render; retrying safely in software.",
          {
            episodeId: request.episodeId,
            encoder: renderInput.hardwareEncoder,
            error: String(error),
          },
        );
        renderInput.hardwareEncoder = undefined;
        await fs.rm(premasterPath, { force: true }).catch(() => undefined);
        await renderPrimary();
      }
    ...
    const isPlayable = await validatePlayableMedia(
      workingOutputPath,
      tools,
      requirements,
    );
    if (!isPlayable) throw new Error("Export output could not be validated.");
    await fs.rename(workingOutputPath, outputPath);
    const cameraOutputs =
      request.type === "editor-handoff" && !request.practice
        ? await createEditorCameraFiles(
            request,
            running,
            sourceDurationMs,
            controller.signal,
    ...
    if (captionSidecar) {
      const reservedCaptions = await nextAvailableExportPath(
        folder,
        request.type === "editor-handoff"
          ? path.join("04 Project Notes", "captions.srt")
          : "captions.srt",
      );
      captionFile = reservedCaptions.fileName;
      await fs.mkdir(path.dirname(reservedCaptions.outputPath), {
        recursive: true,
      });
      await fs.writeFile(reservedCaptions.outputPath, captionSidecar, "utf8");
    }
    const reservedEditDecision = await nextAvailableExportPath(
      folder,
      request.type === "editor-handoff"
        ? path.join("04 Project Notes", "edit-decision-list.json")
        : "edit-decision-list.json",
    );
    const editDecisionFile = reservedEditDecision.fileName;
    await fs.mkdir(path.dirname(reservedEditDecision.outputPath), {
      recursive: true,
    });
    await fs.writeFile(
      reservedEditDecision.outputPath,
      JSON.stringify(
        compactTimelineDraftForPersistence(request.draft),
        null,
        2,
      ),
      "utf8",
    ...
    await writeExportArtifacts(failed);
    report?.(failed);
    await logger.error("ExportService", "FFmpeg export failed.", {
      episodeId: request.episodeId,
      error: String(error),
    });
    return failed;
  } finally {
    await fs.rm(workingOutputPath, { force: true }).catch(() => undefined);
...
export async function cancelExport(
  ...
  await logger.info("ExportService", "Canceled local export job.", {
    episodeId,
  });
  return canceled;
}
