...
import { cancelAutoEdit, runAutoEdit } from "./auto-edit-store";
import { createDiagnosticsBundle, getStorageStatus } from "./diagnostics-store";
import {
  analyzeReviewMediaSync,
  configureMediaPlaybackBaseUrl,
  importReviewMediaFile,
  loadReviewMedia,
  relinkImportedMediaFile,
  verifyImportedMediaIntegrity,
} from "./review-media-store";
import {
  cleanupEpisodeStorage,
  getEpisodeStorageSummary,
} from "./episode-maintenance-store";
import type { ReviewMediaImportSlot } from "../shared/review-media";
import { StudioWindowManager } from "./studio-window-manager";
import {
  startMediaPlaybackServer,
  type MediaPlaybackServer,
} from "./media-playback-server";
...
function createWindow() {
  ...
  mainWindow.on("close", (event) => {
    ...
    const choice = dialog.showMessageBoxSync(mainWindow, {
      ...
      message:
        "Keep What About It Studio open until the recording is stopped and verified.",
      detail:
        "If you exit now, the chunks already written to disk can be recovered next time, but the final seconds may be incomplete.",
...
async function validateRecordingLibraryFolder(folderPath: string) {
  ...
  const probePath = path.join(
    resolved,
    `.what-about-it-write-test-${crypto.randomUUID()}`,
  );
  try {
...
async function getSettings(): Promise<StudioSettings> {
  ...
  const settings = await readJsonFile<StudioSettings>(settingsPath, {
    ...
    defaultEpisodeFolderName:
      defaultStudioConfiguration.storage.episodeFolderName,
...
async function saveSettings(settings: StudioSettings) {
  ...
  const requestedEpisodesRoot = nextSettings.recordingPreferences
    .primaryFolderPath
    ? await validateRecordingLibraryFolder(
        nextSettings.recordingPreferences.primaryFolderPath,
      )
    : undefined;
  ...
  if (getEpisodesRoot() !== previousEpisodesRoot) {
    if (closeProtectedWebContents.size > 0) {
      ...
      throw new Error(
        "Stop the recording before changing the primary recording folder.",
      );
    }
    await ensureBaseFolders();
...
app.whenReady().then(async () => {
  ...
  const requestedEpisodesRoot =
    initialSettings.recordingPreferences?.primaryFolderPath;
  if (requestedEpisodesRoot) {
    try {
      configureEpisodesRoot(
        await validateRecordingLibraryFolder(requestedEpisodesRoot),
      );
    } catch (error) {
      configureEpisodesRoot();
      initialSettings.recordingPreferences = {
        ...defaultRecordingPreferences,
        ...initialSettings.recordingPreferences,
      ...
      await logger.warning(
        "App",
        "The selected recording library was unavailable, so the app returned to default storage.",
        { error: String(error) },
      );
    }
  } else {
  ...
  ipcMain.handle("workspace:save-state", (_event, state) =>
    saveWorkspaceState(state),
  );
  ipcMain.handle("workspace:get-displays", () =>
    studioWindowManager.getDisplays(),
  );
  ipcMain.handle(
  ...
  ipcMain.handle("workspace:close-panel", (_event, panelId: StudioPanelId) =>
    studioWindowManager.closePanel(panelId),
  );
  ipcMain.handle(
    "workspace:move-panel",
    (_event, input: { panelId: StudioPanelId; displayId: number }) =>
      studioWindowManager.movePanel(input.panelId, input.displayId),
  );
  ipcMain.handle(
    "workspace:apply-layout",
    (_event, input: { layoutId: StudioLayoutProfileId; episodeId?: string }) =>
      studioWindowManager.applyLayout(input.layoutId, input.episodeId),
  );
  ipcMain.handle("workspace:reset-layout", () =>
    studioWindowManager.resetLayout(),
  );
  ipcMain.handle("recording:create-session", (_event, input) =>
    createRecordingSession(input),
  );
  ipcMain.handle("recording:write-state", (_event, input) =>
    writeRecordingState(input.folderPath, input.state),
  );
  ipcMain.handle("recording:begin-media", (_event, folderPath) =>
    beginRecordingMedia(folderPath),
  );
  ipcMain.handle("recording:append-chunk", (_event, input) =>
    appendRecordingChunk(input.folderPath, input.chunk),
  );
  ipcMain.handle("recording:finalize-media", (_event, folderPath) =>
    finalizeRecordingMedia(folderPath),
  );
  ipcMain.handle("recording:recover", (_event, folderPath) =>
    recoverRecordingSession(folderPath),
  );
  ipcMain.handle("recording:open-folder", async (_event, folderPath) =>
  ...
  ipcMain.handle("recording:choose-primary-folder", async (event) => {
    ...
    const result = parent
      ? await dialog.showOpenDialog(parent, {
          title: "Choose the primary recording library",
  ...
  ipcMain.handle("recording:choose-backup-folder", async (event) => {
    ...
    const result = parent
      ? await dialog.showOpenDialog(parent, {
          title: "Choose a second recording backup drive",
          properties: ["openDirectory", "createDirectory"],
        })
      : await dialog.showOpenDialog({
          title: "Choose a second recording backup drive",
  ...
  ipcMain.handle("recording:save-program", (_event, input) =>
    saveProgramRecording(input.folderPath, input.bytes),
  );
  ipcMain.handle("recording:save-tracks", (_event, input) =>
    saveRecordedTracks(input.folderPath, input.tracks),
  );
  ipcMain.handle("recording:append-error", (_event, input) =>
    appendRecordingError(input.folderPath, input.message),
  );
  ipcMain.handle("recording:list-unfinished", listUnfinishedRecordingSessions);
  ipcMain.handle("podcast-tools:load", (_event, episodeId) =>
    loadPodcastTools(episodeId),
  );
  ipcMain.handle("podcast-tools:save", (_event, input) =>
    savePodcastTools(input.episodeId, input.state),
  );
  ipcMain.handle("timeline:load", (_event, episodeId) =>
    loadTimelineDraft(episodeId),
  );
  ipcMain.handle("timeline:save", (_event, input) =>
    saveTimelineDraft(input.episodeId, input.draft),
  );
  ipcMain.handle("local-transcription:status", () =>
    getLocalTranscriptionStatus(),
  );
  ipcMain.handle("local-transcription:start", (event, episodeId: string) =>
    transcribeEpisodeLocally(episodeId, (progress) =>
      event.sender.send("local-transcription:progress", progress),
    ),
  );
  ipcMain.handle("local-transcription:cancel", (_event, episodeId: string) =>
    cancelLocalTranscription(episodeId),
  );
  ipcMain.handle("review-media:load", (event, episodeId) =>
    loadReviewMedia(episodeId, {
      onProgress: (progress) =>
        event.sender.send("review-media:preparation-progress", progress),
    }),
  );
  ipcMain.handle(
    "review-media:import",
    async (
      event,
      input: { episodeId: string; slot: ReviewMediaImportSlot },
    ) => {
      const isVideo = input.slot.startsWith("camera-");
      const options = {
        title: isVideo
          ? `Choose ${input.slot.replace("camera-", "Camera ")} video`
      ...
      try {
        return {
          canceled: false,
          inventory: await importReviewMediaFile(
            input.episodeId,
            input.slot,
            result.filePaths[0],
            {
              ...
              onProgress: (progress) =>
                event.sender.send("review-media:import-progress", progress),
            },
          ),
        ...
        if (activeMediaImports.get(importKey) === controller)
          activeMediaImports.delete(importKey);
      }
    },
  );
  ipcMain.handle(
    "review-media:cancel-import",
    (_event, input: { episodeId: string; slot: ReviewMediaImportSlot }) => {
      const active = activeMediaImports.get(`${input.episodeId}:${input.slot}`);
      active?.abort();
      return Boolean(active);
    },
  );
  ipcMain.handle("review-media:auto-sync", (_event, episodeId: string) =>
    analyzeReviewMediaSync(episodeId),
  );
  ipcMain.handle("review-media:verify-originals", (_event, episodeId: string) =>
    verifyImportedMediaIntegrity(episodeId),
  );
  ipcMain.handle(
    "review-media:relink",
    async (
      ...
      const options = {
        ...
        filters: [
          {
            name: isVideo ? "Video files" : "Audio files",
            extensions: isVideo
      ...
      if (result.canceled || !result.filePaths[0])
        return {
          canceled: true,
          inventory: await loadReviewMedia(input.episodeId),
          message: "Relink canceled.",
        };
      return {
        canceled: false,
        inventory: await relinkImportedMediaFile(
          input.episodeId,
          input.slot,
          result.filePaths[0],
        ),
        message: "Protected original relinked and fingerprinted.",
      };
    },
  );
  ipcMain.handle("episode-storage:get", (_event, episodeId: string) =>
    getEpisodeStorageSummary(episodeId),
  );
  ipcMain.handle("episode-storage:cleanup", (_event, input) =>
    cleanupEpisodeStorage(input.episodeId, input.scope),
  );
  ipcMain.handle("review-media:treatment-preview", (_event, input) =>
    renderTrackTreatmentPreview(input),
  );
  ipcMain.handle("auto-edit:run", (event, input) =>
    runAutoEdit(input, (progress) =>
      event.sender.send("auto-edit:progress", progress),
    ),
  );
  ipcMain.handle("auto-edit:cancel", (_event, episodeId: string) =>
    cancelAutoEdit(episodeId),
  );
  ipcMain.handle("export:create", (event, input) =>
    createExport(input, (job) => event.sender.send("export:progress", job)),
  );
  ...
  ipcMain.handle("export:cancel", (_event, input) =>
    cancelExport(input.episodeId, input.job),
  );
  ipcMain.handle("export:open-folder", (_event, input) =>
    openExportFolder(input.episodeId, input.outputFolder),
  );
  ipcMain.handle("diagnostics:create", (_event, input) =>
    createDiagnosticsBundle(input),
  );
