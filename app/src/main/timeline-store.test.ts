...
describe("timeline store", () => {
  beforeEach(async () => {
    mockPaths.episodesRoot = await fs.mkdtemp(
      path.join(os.tmpdir(), "wai-timeline-"),
    );
  });
  ...
  it("saves draft edits locally without changing originals", async () => {
    const { saveTimelineDraft, loadTimelineDraft } =
      await import("./timeline-store");
  ...
  it("refuses to cross-save a draft into another episode", async () => {
    ...
    await expect(saveTimelineDraft("episode-b", draft)).rejects.toThrow(
      "Refusing to save draft for episode-a into episode episode-b",
    );
  });

  it("persists a bounded undo window so edits can be reversed after restart", async () => {
    const { saveTimelineDraft, loadTimelineDraft } =
      await import("./timeline-store");
    const draft = applyTimelineEdit(
      selectTimelinePoint(
